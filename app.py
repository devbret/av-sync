...
def analyze_audio_segments(
    ...
    return {"sr": sr, "duration": duration, "segments": out, "curves": curves}
...
def score_all_videos(videos_dir: str, motion_method: str = "flow") -> List[Dict]:
    ...
    results = []
    for p in tqdm(paths, desc=f"Scoring video motion ({motion_method})"):
        results.append({"path": p, "motion_score": motion_score_video(p, method=motion_method)})
    ...
    for i, t in enumerate(times):
        ...
        if not ok:
            if fps > 0:
                ...
                frame_idx = int(round(t * (frames / max(dur, 1e-9))))
        ...
        if prev_gray is not None:
            if method == "flow":
                scores[i] = _flow_mag(prev_gray, gray)
            else:
                scores[i] = _diff_score(prev_gray, gray)
        prev_gray = gray
...
def _interp_curve(times_src: np.ndarray, values_src: np.ndarray, times_q: np.ndarray) -> np.ndarray:
    ...
    return np.interp(times_q, times_src.astype(float), values_src.astype(float), left=float(values_src[0]), right=float(values_src[-1]))
...
def pick_region_by_timeline_windowed(
    ...
    for i, idx in enumerate(w_idx):
        ...
        score = base_weight * d_basic + curve_weight * d_curve
...
def choose_clips_for_segments(
    ...
    def ensure_meta(vpath: str, motion_score: float):
        ...
        tl = motion_timeline(vpath, step_s=vt_step, max_samples=vt_max_samples, method=motion_method) if clip_strategy == "timeline" else {"times": np.array([]), "scores": np.array([])}
...
def main():
    ...
    ap.add_argument("--motion-method", choices=["flow", "diff"], default="flow", help="Motion scoring method.")
    ...
    ap.add_argument("--curve-weight", type=float, default=0.60, help="0..1 weight for curve correlation vs pace/burst match.")
    ...
    scored = score_all_videos(args.videos_dir, motion_method=args.motion_method)
